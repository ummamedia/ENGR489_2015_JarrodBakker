# Test 2: There should be rules which block traffic between the hosts,
#         however this test will be invoking connections which are not
#         blocked. The purpose of this is to ensure that rules do not
#         traffic that they should not block. Connectivity will be
#         tested through ping requests.
# Usage: python test2_rulesNoBlock.py <target host in dotted decimal> <number of pings>
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print "ERROR: Usage: python test2_rulesNoBlock.py <target host in dotted decimal> <number of pings>"
        sys.exit(2)
    logging.basicConfig(filename="test_out.log",
                        format='%(asctime)s %(message)s',
                        level=logging.DEBUG)
    rulesNoBlock(str(sys.argv[1]), int(sys.argv[2]))
